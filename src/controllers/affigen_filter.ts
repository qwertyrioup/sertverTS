...
export const getAll = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const filters: IAffigenFilter[] | [] = await AffigenFilter.find();
  ...
  export const getAllFiltersElastic = async (req: Request, res: Response, next: NextFunction) => {
    try {
    ...
    //   @ts-ignore
      const data = result.aggregations.all_filters.filter_names.buckets.map(
        (obj: any) => {
          ...
          const sortedFilterValues = obj.filter_values.buckets.sort((a: any, b: any) =>
  ...
  export const insertParentFilter = async (req: Request, res: Response, next: NextFunction) => {
    const data = req.body; // Data for the new parent filter document
    try {
      const newFilter = new AffigenFilter(data); // Create a new document instance
  ...
  export const insertSubFilter = async (req: Request, res: Response, next: NextFunction) => {
    const id = req.params.id;
    const data = req.body.data;
    const newObj = { filter_value: data.filter_value };

    try {
      // Check if the filter object already exists
      const existingFilter = await AffigenFilter.findOne({
        _id: id,
        "counts.filter_value": newObj.filter_value
      });

      if (!existingFilter) {
        // Add new object only if it doesn't exist
        const filter = await AffigenFilter.findByIdAndUpdate(id, {
          $addToSet: { counts: newObj }
        }, { new: true });
  ...
  export const deleteParentFilter = async (req: Request, res: Response, next: NextFunction) => {
    const id = req.params.id; // ID of the parent filter to delete

    try {
      const result = await AffigenFilter.findByIdAndDelete(id); // Delete the document
      if (result) {
  ...
  export const deleteSubFilter = async (req: Request, res: Response, next: NextFunction) => {
    const parentId = req.params.parentId;
    const subFilterId = req.params.subFilterId;

    try {
      const filter = await AffigenFilter.findByIdAndUpdate(
        parentId,
        {
  ...
  export const updateFilterChildLogic = async (req: Request, res: Response, next: NextFunction) => {
    try {
      let response;
      const { filterId, filterName, additionalData, operator, queryData } = req.body.data;
      ...
      const filter = await AffigenFilter.findById(filterId);
      if (!filter) {
         next(createError(404, 'Filter not found'));
      }
    //   @ts-ignore
    ...
    //   @ts-ignore
      ...
      await AffigenFilter.findByIdAndUpdate(
        filterId,
  ...
  export const getFilter = async (req: Request, res: Response, next: NextFunction) => {
    try {
      // Retrieve the filter document by ID
      let filter = await AffigenFilter.findById(req.params.id);
  ...
  export const applyLogicForAll = async (req: Request, res: Response, next: NextFunction) => {
  ...
  jsonStream.pipe(res);
    try {
      ...
      if (filters.length > 0) {
        ...
        await AffigenProduct.updateMany({}, {$set: {filters: [], sync: false}});
